...
import icedSpanishLatte from './assets/iced-spanish-latte.jpeg'
      ...
      <header className="fixed top-0 left-0 w-full bg-white shadow z-50">
        <div className="flex justify-between items-center p-4 max-w-6xl mx-auto">
          ...
          <nav className="hidden md:flex space-x-6">
            <button onClick={() => scrollTo('menu')} className="hover:text-black text-gray-600">Menu</button>
            <button onClick={() => scrollTo('vision')} className="hover:text-black text-gray-600">Vision</button>
      ...
      <main className="pt-20">
        ...
        <section id="home" className="h-screen bg-cover bg-center flex flex-col justify-center items-center text-center" style={{ backgroundImage: `url(${heroImage})` }}>
          <div className="bg-white bg-opacity-80 p-6 rounded">
            <h2 className="text-5xl font-bold mb-4">Welcome to DopaBeans</h2>
            <p className="text-lg text-gray-600">Crafted Coffee. Honest Taste.</p>
          </div>
        </section>
            ...
            <div>
              ...
              <ul className="flex overflow-x-auto space-x-4 snap-x snap-mandatory px-1 pb-2">
                {[
                  ...
                  <li key={i} className="min-w-[200px] snap-start shrink-0 bg-gray-100 p-4 rounded shadow text-left">
                    <img src={item.img} alt={item.name} className="mb-2 rounded h-32 w-full object-cover" />
                    {item.name}
                  </li>
                ))}
              </ul>
        ...
        {/* Vision */}
        <section id="vision" className="py-20 bg-gray-100 text-center">
        ...
        {/* Footer */}
        <footer id="contact" className="bg-black text-fontDarkBackground py-12 text-center">
          <h3 className="text-lg font-semibold mb-2">Contact Us</h3>
          <p className="text-sm">dopabeans@dopabeansuae.com</p>
          <p className="text-sm">Instagram: @dopabeansuae</p>
        </footer>
      </main>
    </div>
  )
